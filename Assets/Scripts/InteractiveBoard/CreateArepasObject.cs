...
public class CreateArepasObject : MonoBehaviour
{
     CircleCollider2D circleCollider; // Reference to the Circle Collider
    [SerializeField] StoveFoodContainer stoveFoodContainer; // Reference to the Circle Collider
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0)) // 0 for left mouse button
        {
            ...
            if (circleCollider.OverlapPoint(mousePosition))
            {
                ...
                stoveFoodContainer.CreateArepas( );
